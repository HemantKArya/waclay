...
use wasm_component_layer_util::{DiscriminantSize, FlagsSize};
...
mod kw {
    ...
    syn::custom_keyword!(wasm_component_layer_crate);
}
...
impl Parse for ComponentAttr {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        } else if lookahead.peek(kw::wasm_component_layer_crate) {
            input.parse::<kw::wasm_component_layer_crate>()?;
            input.parse::<Token![=]>()?;
...
pub trait Expander {
    fn expand_record(
        ...
        fields: &[&syn::Field],
        wasm_component_layer_crate: &syn::Path,
    ) -> Result<TokenStream>;
...
pub fn expand(expander: &dyn Expander, input: &DeriveInput) -> Result<TokenStream> {
    let mut wasm_component_layer_crate = None;
    let mut style = None;
    ...
    for attribute in &input.attrs {
        ...
        match attribute.parse_args()? {
            ComponentAttr::WasmtimeCrate(c) => wasm_component_layer_crate = Some(c),
    ...
    let wasm_component_layer_crate = wasm_component_layer_crate.unwrap_or_else(default_wasm_component_layer_crate);
    match style {
        Style::Record => expand_record(expander, input, &wasm_component_layer_crate),
        Style::Enum | Style::Variant => expand_variant(expander, input, style, &wasm_component_layer_crate),
    }
}

fn default_wasm_component_layer_crate() -> syn::Path {
    Ident::new("wasm_component_layer", Span::call_site()).into()
}
...
fn expand_variant(
    ...
    match style {
        ...
        Style::Enum => {
            ...
            expander.expand_enum(&input.ident, discriminant_size, &cases, wasm_component_layer_crate)
        }
        Style::Record => unreachable!(),
    }
}
...
pub fn expand_flags(flags: &Flags) -> Result<TokenStream> {
    let wt = default_wasm_component_layer_crate();
